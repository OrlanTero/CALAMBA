...
class Connection
{
    ...
    protected $DATABASE = "cmdc";

    protected $USERNAME = "cmdc";

    protected $PASSWORD = "115314450Oshe";
    ...
    public function SelectPage($table, $where, $fetchAll, $start = 1, $limit = 10, $order = "")
    {
        $w = isset($where) ? " WHERE " . $this->ConditionToQ($where, " AND ", "=") : "";
        $query = "SELECT * FROM " . $table . $w . " " . $order . " LIMIT $limit OFFSET $start";   
    ...
    public function SelectMultiConditionInnerJoin($table, $innerJoin, $condition,  $fetchAll, $limit = false, $order = "", $specific = "*")
    {
        $w = $this->MultiConditionToQ($condition, " AND ");
        $w = empty($condition) ? "" : " WHERE " . $w;
        $l = $limit ? "LIMIT " . $limit : "";
        $query = implode(" ", array("SELECT " . $specific . " FROM " . $table . " " . $innerJoin . " " . $w, $order, $l));
...
//        return $fetchAll ? $stmt->fetchAll() : $stmt->fetch();
    ...
    public function SelectOr($table, $options, $fetchAll, $order = "")
    {
    ...
    public function SelectPageOr($table, $options, $fetchAll, $start = 1, $limit = 10, $order = "")
    {
        $w = !empty($options) ? " WHERE " . $this->ConditionToQOption($options) : "";
        $query = "SELECT * FROM " . $table . $w . " " . $order . " LIMIT $limit OFFSET $start";
        $stmt = $this->CONNECTION->prepare($query);
    ...
    public function SelectBetweenOr($table, $column, $from, $to, $fetchAll, $limit = false, $order = "", $where,  $specific = "*")
    {
        $l = $limit ? "LIMIT " . $limit : "";
        $w  = isset($where) ? $this->ConditionToQOption($where, " OR ") . " AND " : "";
        $query = implode(" ", array("SELECT " . $specific . " FROM ", $table, "WHERE", $w, "DATE(" . $column . ")", "BETWEEN", "'" . $from . "'", "AND", "'" . $to . "'", $order, $l));
        $stmt = $this->CONNECTION->prepare($query);
        $stmt->execute();
    ...
    public function SelectBetweenPageOr($table, $column, $from, $to, $fetchAll, $start = 0, $limit = 10, $where, $order = "", $specific = "*")
    {
        $w  = isset($where) ? $this->ConditionToQOption($where, " OR ") . " AND " : "";
        $query = implode(" ", array("SELECT " . $specific . " FROM ", $table, "WHERE", $w, "DATE(" . $column . ")", "BETWEEN", "'" . $from . "'", "AND", "'" . $to . "'", $order, "LIMIT", $limit, "OFFSET", $start));
        $stmt = $this->CONNECTION->prepare($query);
        $stmt->execute();
    ...
    public function SelectBetween($table, $column, $from, $to, $fetchAll, $limit = false, $order = "", $where,  $specific = "*")
    {
        $l = $limit ? "LIMIT " . $limit : "";
        $w  = isset($where) ? $this->ConditionToQ($where, " AND ") . " AND " : "";
        $query = implode(" ", array("SELECT " . $specific . " FROM ", $table, "WHERE", $w, "DATE(" . $column . ")", "BETWEEN", "'" . $from . "'", "AND", "'" . $to . "'", $order, $l));
        $stmt = $this->CONNECTION->prepare($query);
        $stmt->execute();
    ...
    public function SelectPageBetween($table, $column, $from, $to, $fetchAll, $start = 0, $limit = 10, $where, $order= "", $specific = "*")
    {
        $w  = isset($where) ? $this->ConditionToQ($where, " AND ") . " AND " : "";
        $query = implode(" ", array("SELECT " . $specific . " FROM ", $table, "WHERE", $w, "DATE(" . $column . ")", "BETWEEN", "'" . $from . "'", "AND", "'" . $to . "'", $order, "LIMIT", $limit, "OFFSET", $start));
        $stmt = $this->CONNECTION->prepare($query);
    ...
    public function SearchJoin($table, $join, $search, $into, $where, $fetchAll = true)
    {
        $query = $this->MakeSearchQuery($table, $search, $into, $where, $join);
        $stmt = $this->CONNECTION->prepare($query);
        $stmt->execute();
        return $fetchAll ? $stmt->fetchAll() : $stmt->fetch();
    }
    ...
    public function MakeSearchQuery($table, $search, $into, $where, $join = "")
    {
        $w = isset($where) ? $this->ConditionToQ($where, " AND ") : "";
        ...
        return "SELECT * FROM " . $table . " " . $join . " " .( implode("", array_map(function ($record) use($table) {
